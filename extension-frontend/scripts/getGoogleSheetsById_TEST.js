...
async function showInsertModalForm1() {
    ...
    h3.textContent = "Enter GoogleSheet ID";
    h3.style.cssText = `
        font-size: 24px;
        color: #333;
        margin-bottom: 30px;
    ...
    manualInput.id = "manualSpreadsheetId";
    manualInput.placeholder = "Enter Spreadsheet ID";
    ...
    manualInput.onmouseover = () => manualInput.style.borderColor = '#007bff';
    manualInput.onmouseout = () => manualInput.style.borderColor = '#e1e1e1';

    // Контейнер для полей диапазона
    ...
    sheetNameInput.id = "sheetName";
    sheetNameInput.placeholder = "Sheet name (e.g., Sheet1)";
    sheetNameInput.required = true;
    sheetNameInput.style.cssText = manualInput.style.cssText;
    sheetNameInput.style.flex = "1";

    // Input для диапазона
    const rangeInput = document.createElement("input");
    rangeInput.type = "text";
    rangeInput.id = "range";
    rangeInput.placeholder = "Range (e.g., A2:A10)";
    rangeInput.required = true;
    rangeInput.style.cssText = manualInput.style.cssText;
    rangeInput.style.flex = "1";

    sheetNameInput.onmouseover = () => sheetNameInput.style.borderColor = '#007bff';
    sheetNameInput.onmouseout = () => sheetNameInput.style.borderColor = '#e1e1e1';

    rangeInput.onmouseover = () => rangeInput.style.borderColor = '#007bff';
    rangeInput.onmouseout = () => rangeInput.style.borderColor = '#e1e1e1';

    rangeContainer.appendChild(sheetNameInput);
    ...
    p.textContent = "** A spreadsheet ID can be extracted from its URL. For example, the spreadsheet ID in the URL https://docs.google.com/spreadsheets/d/abc1234567/edit#gid=0 is 'abc1234567'.";
    ...
    form.addEventListener('submit', async (e) => {
        e.preventDefault();
        const spreadsheetId = manualInput.value.trim();
        const sheet_name = sheetNameInput.value.trim();
        const range = rangeInput.value.trim();

        if (spreadsheetId) {
            const flag = isValidSpreadsheetId(spreadsheetId);
            if (!flag) {
                return;
            }
            ...
            const data = await getEmailsFromGoogleSheet(spreadsheetId, fullRange);
            await openNewMessageInGmail(data);
        }
...
function isValidSpreadsheetId(input) {
    const pattern = /^[a-zA-Z0-9-_]{40,100}$/; // Проверяем длину и допустимые символы
