...
async function getFiles(composeWindow) {
    const fullSizeWindowParent = document.querySelector("div.aSs");

      const style = getComputedStyle(fullSizeWindowParent);
      console.log();
      if (style.visibility !== "hidden") {
        const fullSizeWindow = fullSizeWindowParent.querySelector("div.aSt");
        composeWindow = fullSizeWindow;
      }
...
async function getEmailData(composeWindow) {
    ...
    try {
        console.log("composeWindow: ", composeWindow);
        console.log("Classes of composeWindow:", composeWindow.classList);
        // Get recipients from the specific compose window
        const recipientNodes = composeWindow.querySelectorAll('.afV');
        console.log("recipientsNodes: ", recipientNodes);
        ...
        for (const [index, rep] of emailData.recipients.entries()) {
            if (typeof rep === 'string' && rep.includes('recipients') && rep.includes('quicksend')) {
                ...
                // Удаляем найденную строку из массива
                emailData.recipients.splice(index, 1);  // Удаляем текущий элемент
        ...
        // Get subject from the specific compose window
        const subjectField = composeWindow.querySelector('input[name="subjectbox"]');
        emailData.subject = subjectField ? subjectField.value : '';

        // Get message body from the specific compose window
        const messageBody = composeWindow.querySelector('div[role="textbox"][contenteditable="true"]');
        ...
        if (timeZone) {
          ...
          console.log(`Таймзона из хранилища: ${timeZone}`);
        } else {
          console.log('Таймзона из хранилища не найдена');
        }
        ...
        if (inputDateTime < now) {
          ...
          emailData.date = date ? date.value : '';
          emailData.time = time ? time.value : '';
        }
...
function checkForComposeWindows() {
    const composeWindows = document.querySelectorAll('.aSt, .AD');
...
async function startCampaign(data, composeWindow) {
    try {
      ...
      await createModalWindow(result["message"]);
      return result;
    } catch (error) {
